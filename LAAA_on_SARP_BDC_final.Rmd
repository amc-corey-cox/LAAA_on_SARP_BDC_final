...
imputed_laaa_results <- read_tsv("results/laaa_chr19_imputed_maxFEV1_reversal_laaa.txt")
wgs_laaa_results <- read_tsv("results/laaa_chr19_wgs_maxFEV1_reversal_laaa.txt")

meta_laaa_results <- imputed_laaa_results %>%
  inner_join(wgs_laaa_results, by = c("position", "ref", "alt"), suffix = c("_imputed", "_wgs")) %>%
  select(position, ref, alt, contains(c("alt_freq", "n_", "un", "beta"))) %>%
# Have to use rowwise because of how the variables are grouped.
  rowwise %>%
  mutate(meta_afr_dose_beta = getCombinedBeta(c(afr_dose_beta_imputed, afr_dose_beta_wgs), c(afr_dose_beta_se_imputed, afr_dose_beta_se_wgs)),
         meta_afr_dose_se = getCombinedStdErr(c(afr_dose_beta_se_imputed, afr_dose_beta_se_wgs)),
         meta_afr_does_p = getP(meta_afr_dose_beta, meta_afr_dose_se),
