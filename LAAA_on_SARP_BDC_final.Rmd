...
get_meta_results <- function(.x, .y) {
  ...
  select(position, ref, alt, contains(c("alt_freq", "n_", "un", "beta"))) %>%
# Have to use rowwise because of how the variables are grouped.
  ...
  mutate(.keep = "unused",
    ...
    meta_afr_dose_beta = getCombinedBeta(c(afr_dose_beta_imputed, afr_dose_beta_wgs), c(afr_dose_beta_se_imputed, afr_dose_beta_se_wgs)),
...
meta_SARP1_2_maxFEV1_reversal_chr19 <- get_meta_results(
...
meta_SARP1_2_maxFEV1_reversal_chr9 <- get_meta_results(
...
meta_SARP1_2_FVC_chr16 <- get_meta_results(
...
meta_SARP1_2_FVC_chr13 <- get_meta_results(
...
meta_SARP1_2_2puffs_chr11 <- get_meta_results(
...
meta_SARP1_2_maxFVC_chr8 <- get_meta_results(
...
meta_SARP1_2_maxFEV1_chr1 <- get_meta_results(
...
hits_meta_SARP1_2_maxFEV1_reversal_chr19 <- meta_SARP1_2_maxFEV1_reversal_chr19 %>%
  filter(meta_allele_dose_p < 0.001) %>%
  select(1:5, contains("meta")) %>%
  mutate(chr = 19, .before = 1)

hits_meta_SARP1_2_maxFEV1_reversal_chr9 <- meta_SARP1_2_maxFEV1_reversal_chr9 %>%
  filter(meta_allele_dose_p < 0.001) %>%
  select(1:5, contains("meta")) %>%
  mutate(chr = 9, .before = 1)

hits_meta_SARP1_2_FVC_chr16 <- meta_SARP1_2_FVC_chr16 %>%
  filter(meta_allele_dose_p < 0.001) %>%
  select(1:5, contains("meta")) %>%
  mutate(chr = 16, .before = 1)

hits_meta_SARP1_2_FVC_chr13 <- meta_SARP1_2_FVC_chr13 %>%
  filter(meta_allele_dose_p < 0.001) %>%
  select(1:5, contains("meta")) %>%
  mutate(chr = 13, .before = 1)

hits_meta_SARP1_2_2puffs_chr11 <- meta_SARP1_2_2puffs_chr11 %>%
  filter(meta_allele_dose_p < 0.001) %>%
  select(1:5, contains("meta")) %>%
  mutate(chr = 11, .before = 1)

hits_meta_SARP1_2_maxFVC_chr8 <- meta_SARP1_2_maxFVC_chr8 %>%
  filter(meta_allele_dose_p < 0.001) %>%
  select(1:5, contains("meta")) %>%
  mutate(chr = 8, .before = 1)

hits_meta_SARP1_2_maxFEV1_chr1 <- meta_SARP1_2_maxFEV1_chr1 %>%
  filter(meta_allele_dose_p < 0.001) %>%
  select(1:5, contains("meta")) %>%
  mutate(chr = 1, .before = 1)

all_hits <- bind_rows(hits_meta_SARP1_2_2puffs_chr11, hits_meta_SARP1_2_FVC_chr13, hits_meta_SARP1_2_FVC_chr16,
                  hits_meta_SARP1_2_maxFEV1_chr1, hits_meta_SARP1_2_maxFEV1_reversal_chr19,
                  hits_meta_SARP1_2_maxFEV1_reversal_chr9, hits_meta_SARP1_2_maxFVC_chr8)
