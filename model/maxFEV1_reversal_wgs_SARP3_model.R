...
runLaaaModelSummary <- function(model.data.frame) {
  ...
  return (summary(lm(log(max_fev1_reversal + 1) ~ age + sex + height_cm + base_FEV1 + allele_dose + afr_dose + allele_afr_dose, data=model.data.frame)))
}

runNullModel <- function(model.data.frame) {
  # return (lm(Y ~ covariates, data=model.data.frame))
  return (lm(log(max_fev1_reversal + 1) ~ age + sex + height_cm + base_FEV1, data=model.data.frame))
}

runLaaaModel <- function(model.data.frame) {
  # return (lm(Y ~ covariates + allele_dose + afr_dose + allele_afr_dose, data=model.data.frame))
  return (lm(log(max_fev1_reversal + 1) ~ age + sex + height_cm + base_FEV1 + allele_dose + afr_dose + allele_afr_dose, data=model.data.frame))
}

runAlleleModelSummary <- function(model.data.frame) {
  # return (summary(lm(Y ~ covariates + allele_dose, data=model.data.frame)))
  return (summary(lm(log(max_fev1_reversal + 1) ~ age + sex + height_cm + base_FEV1 + allele_dose, data=model.data.frame)))
}